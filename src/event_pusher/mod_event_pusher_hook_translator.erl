...
-export([filter_local_packet/1,
	 unacknowledged_message/2, user_not_present/5,
...
-spec start(Host :: jid:server(),
	    Opts :: proplists:proplist()) -> any().

start(Host, _Opts) -> ejabberd_hooks:add(hooks(Host)).

-spec stop(Host :: jid:server()) -> ok.

stop(Host) -> ejabberd_hooks:delete(hooks(Host)).

%%--------------------------------------------------------------------
%% Hook callbacks
%%--------------------------------------------------------------------
-type routing_data() :: {jid:jid(), jid:jid(),
			 mongoose_acc:t(), exml:element()}.

-spec filter_local_packet(drop) -> drop;
			 (routing_data()) -> routing_data().

filter_local_packet(drop) -> drop;
filter_local_packet({From, To = #jid{lserver = Host},
		     Acc0, Packet}) ->
...
-spec user_send_packet(mongoose_acc:t(),
    ...
    case chat_type(Acc) of
      ...
      Type ->
        Event = #chat_event{type = Type, direction = in,
                    from = From, to = To, packet = Packet},
        NewAcc = mod_event_pusher:push_event(Acc,
...
user_send_packet(Acc, From, To,
    ...
    case  exml_query:paths(Packet, [{element, <<"message">>}]) of
        undefined ->  
            Acc;
        Message ->
            Event = #pubsub_event{from = From, to = To, packet = Message},
            ?LOG_INFO(#{what => subelement, message => Message, event => Event}),
            NewAcc = mod_event_pusher:push_event(Acc, From#jid.lserver, Event),
            merge_acc(Acc, NewAcc)
    end;

user_send_packet(Acc, _From, _To, _Packet) -> Acc.

-spec user_present(mongoose_acc:t(),
		   UserJID :: jid:jid()) -> mongoose_acc:t().

user_present(Acc, #jid{} = UserJID) ->
    Event = #user_status_event{jid = UserJID,
			       status = online},
    NewAcc = mod_event_pusher:push_event(Acc,
					 UserJID#jid.lserver, Event),
    merge_acc(Acc, NewAcc).

-spec user_not_present(mongoose_acc:t(),
		       User :: jid:luser(), Server :: jid:lserver(),
		       Resource :: jid:lresource(),
		       Status :: any()) -> mongoose_acc:t().

user_not_present(Acc, LUser, LHost, LResource,
		 _Status) ->
...
-spec chat_type(mongoose_acc:t()) -> chat | groupchat |
				     headline | normal | false.
...
-spec merge_acc(mongoose_acc:t(),
		mongoose_acc:t()) -> mongoose_acc:t().
